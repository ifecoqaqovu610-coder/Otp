...
# --- Configuration ---
TOKEN = "8466050585:AAFv1Ut-GsWrUStLFaaXVgfFQ1G24vbsqN0"  
ADMIN_ID = 8502686983
BASE_URL = "http://185.190.142.81"
NEXA_API_KEY = "nxa_4cab624d390f5a6e03b1dc364fbdb68b6828b6ea"
ENGINE2_BASE = f"{BASE_URL}/api/user/p2"
...
# --- Country Flags Dictionary (185+ Countries) ---
COUNTRY_FLAGS = {
...
SERVICE_SHORTS = {
    "facebook": "FB", "whatsapp": "WA", "telegram": "TG", "instagram": "IG",
    "twitter": "TW", "x": "X", "google": "GO", "gmail": "GM", "youtube": "YT",
    "apple": "AP", "microsoft": "MS", "tiktok": "TT", "snapchat": "SC",
    "binance": "BN", "melbet": "MB", "bkash": "BK", "rocket": "RK", "nagad": "NG"
}
...
EMOJI_COLLECTION = {
    ...
    "binance": "💰", "melbet": "🎰", "bkash": "💳", "rocket": "🚀", "nagad": "📲",
    "done": "✅", "cross": "❌", "warning": "⚠️", "time": "⏰", "waiting": "🔄",
    "message": "📩", "otp": "🔐", "number": "📞", "world": "🌐", "user": "👤",
    "bot": "🤖", "live": "🟢", "off": "🔴", "traffic": "📊", "star": "⭐",
    "crown": "👑", "fire": "🔥", "globe": "🌍", "pin": "📌", "note": "📝",
    "gear": "⚙️", "link": "🔗", "plus": "➕", "trash": "🗑️", "shield": "🛡️",
    "key": "🔑", "bell": "🔔", "rocket": "🚀", "trophy": "🏆", "lightning": "⚡",
    "bulb": "💡", "tools": "🛠️", "package": "📦", "mega": "📢", "hi": "👋"
}
...
def mask_number(phone):
    ...
    if len(phone_str) > 7: 
        return f"{phone_str[:3]}XXX{phone_str[-4:]}"
    return phone_str
...
def get_force_join_menu():
    ...
    for idx, link in enumerate(channels):
        markup.add(InlineKeyboardButton(f"❌ Remove: {link[:30]}...", callback_data=f"delfjc_{idx}"))
...
def show_user_services(chat_id, message_id=None):
    ...
    if buttons: 
        ...
        markup.add(InlineKeyboardButton(text="ℹ️ No Services Available", callback_data="ignore"))
...
def show_traffic_search(chat_id, message_id=None):
    ...
    text = (
        ...
        f"━━━━━━━━━━━━━━━━━━\n"
        f"Type service name (e.g. WhatsApp, Facebook)\n\n"
        f"📝 Send /cancel to stop"
    )
    if not message_id:
        msg = safe_send(chat_id, text, markup)
        if msg: 
            bot.register_next_step_handler(msg, process_api_traffic_search)
    else:
        safe_send(chat_id, text, markup, message_id)
...
def show_user_countries(chat_id, srv_id, message_id=None):
    ...
    if not srv_data: 
        show_user_services(chat_id, message_id)
        return
    markup = InlineKeyboardMarkup(row_width=2)
    buttons = []
    ...
    if buttons:
        ...
        markup.add(InlineKeyboardButton(text="ℹ️ No Countries Available", callback_data="ignore"))
...
def show_user_ranges(chat_id, srv_id, cnt_id, message_id=None):
    ...
    if not cnt_data: 
        show_user_countries(chat_id, srv_id, message_id)
        return
    markup = InlineKeyboardMarkup(row_width=2)
    flag = get_country_flag(cnt_data['name'])
    buttons = [InlineKeyboardButton(text=f"📱 {rng_val}", callback_data=f"usr_r|{srv_id}|{cnt_id}|{rng_id}") 
               for rng_id, rng_val in cnt_data.get("ranges", {}).items()]
    if buttons:
        markup.add(*buttons)
    else:
        markup.add(InlineKeyboardButton(text="ℹ️ No Ranges Available", callback_data="ignore"))
...
def show_admin_countries(chat_id, srv_id, message_id=None):
    ...
    srv_data = data.get("services_data", {}).get(srv_id)
    if not srv_data: 
        show_admin_services(chat_id, message_id)
        return
    markup = InlineKeyboardMarkup(row_width=2)
    for cnt_id, cnt in srv_data.get("countries", {}).items(): 
        flag = get_country_flag(cnt['name'])
...
def show_admin_ranges(chat_id, srv_id, cnt_id, message_id=None):
    ...
    cnt_data = srv_data.get("countries", {}).get(cnt_id) if srv_data else None
    if not cnt_data: 
        show_admin_countries(chat_id, srv_id, message_id)
        return
    flag = get_country_flag(cnt_data['name'])
    markup = InlineKeyboardMarkup(row_width=1)
    for rng_id, rng_val in cnt_data.get("ranges", {}).items(): 
...
def handle_query(call):
    ...
    if call.data == "check_join":
        if check_force_join(user_id):
            bot.answer_callback_query(call.id, "✅ Welcome to VIP Number Club!", show_alert=True)
            try: 
    ...
    # Number selection and change
        ...
        if msg_obj: 
            fetch_number(chat_id, service_info, NEXA_API_KEY, msg_obj.message_id, is_custom)
    ...
    # Admin callbacks
        ...
        safe_send(chat_id, f"{emo('number')} <b>Send Range (e.g., 8801XXX):</b>", markup, msg_id)
        ...
        safe_send(chat_id, f"{emo('note')} Send new Watermark:\nCurrent: {data.get('watermark', 'VIP NUMBER CLUB')}", markup, msg_id)
        bot.register_next_step_handler_by_chat_id(chat_id, process_set_watermark, msg_id)
    elif call.data == "admin_broadcast":
        markup = InlineKeyboardMarkup().add(InlineKeyboardButton("🔙 Cancel", callback_data="back_to_admin"))
        safe_send(chat_id, f"{emo('message')} Send message to broadcast:\n(Will be sent to ALL users)", markup, msg_id)
...
def process_custom_range(message, msg_id):
    if message.text == '/cancel': 
        try: 
            bot.edit_message_text(f"{emo('cross')} <b>Cancelled.</b>", message.chat.id, msg_id, parse_mode="HTML")
        except: 
            pass
        return
    custom_input = message.text.strip()
    service_info = {'id': f"custom_{custom_input}", 'service_name': "Custom Search", 'country_name': 'Universal', 'range': custom_input, 'srv_id': None, 'cnt_id': None}
    if str(message.chat.id) in active_polls: 
        active_polls[str(message.chat.id)] = False
...
def process_api_traffic_search(message):
    if message.text == '/cancel': 
        safe_send(message.chat.id, f"{emo('cross')} <b>Cancelled.</b>")
        return
    ...
    if user_id in traffic_cooldowns:
        ...
        if time_passed < 10:
            ...
            markup = InlineKeyboardMarkup().add(InlineKeyboardButton("❌ Close", callback_data="close_menu"))
            safe_send(message.chat.id, f"{emo('time')} Wait {wait_time}s before checking again.", markup)
            return
            
    traffic_cooldowns[user_id] = current_time
    service_query = message.text.strip().lower()
    
    safe_send(message.chat.id, f"{emo('waiting')} <b>Checking traffic...</b>")
    ...
    def check_traffic():
        ...
        try:
            ...
            # Get logs from API
            response = requests.get(f"{BASE_URL}/api/v1/console/logs?limit=100", headers=headers, timeout=15)
        ...
        if ranges:
            ...
            res_text = f"{emo('cross')} <b>No active traffic found for {service_query.title()}.</b>"
        safe_send(message.chat.id, res_text, markup)
    ...
    try:
        ...
        if res_data.get("success"):
            ...
            if "balance" in api_err.lower(): 
                api_err = "Insufficient balance. Contact admin."
            markup = InlineKeyboardMarkup()
            if is_custom: 
                markup.add(InlineKeyboardButton("❌ Close", callback_data="close_menu"))
            else: 
...
def poll_otp(chat_id, number_id, phone_number, service_info, api_key, msg_id, is_custom):
    ...
    while time.time() - start_time < timeout:
        ...
        try:
            ...
            if s_data.get("success") and s_data.get("otp"):
                ...
                # Update main message
                success_text = (
                    f"{emo('done')} <b>OTP RECEIVED</b> {emo('done')}\n"
                    f"━━━━━━━━━━━━━━━━━━\n"
                    f"{srv_emoji} Service: <b>{html.escape(service_info['service_name'])}</b>\n"
                    f"{flag} Country: <b>{html.escape(service_info['country_name'])}</b>\n"
                    f"━━━━━━━━━━━━━━━━━━\n"
                    ...
                    f"━━━━━━━━━━━━━━━━━━\n"
                    f"{emo('star')} VIP NUMBER CLUB {emo('star')}"
                )
                markup = InlineKeyboardMarkup()
                if is_custom: 
                    markup.add(InlineKeyboardButton("❌ Close", callback_data="close_menu"))
                ...
                for grp in data.get("forward_groups", []):
                    try:
                        ...
                        if HAS_COPY_BTN:
                            copy_btn = CopyTextButton(text=str(otp_code))
                            grp_markup.add(InlineKeyboardButton(text=f"📋 Copy OTP", copy_text=copy_btn))
                        else:
    ...
    if active_polls.get(str(chat_id), False):
        ...
        if is_custom: 
            ...
            markup.add(InlineKeyboardButton("🔙 Back", callback_data="back_to_user_services"))
        safe_send(chat_id, f"{emo('time')} <b>Timeout!</b> No OTP received within 10 minutes.", markup, msg_id)
        active_polls[str(chat_id)] = False

if __name__ == "__main__":
    print("👑 VIP NUMBER CLUB - Premium Bot Started! 👑")
